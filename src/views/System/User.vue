<template>
  <div>用户管理</div>
</template>

<!-- <template>
  <Content title="用户管理">
    <el-button type="primary" class="break">
      <el-icon><RefreshRight /></el-icon>
      刷新
    </el-button>
    <el-divider />
    <el-button type="primary" class="add"> 添加用户 </el-button>
    <el-button type="danger" class="del"> 删除用户 </el-button>
    <el-table :data="tableData" border style="width: 100%; margin-bottom: 10px">
      <el-table-column align="center" type="selection" width="100" />
      <el-table-column align="center" prop="date" label="ID" width="100" />
      <el-table-column align="center" prop="user" label="用户名" />
      <el-table-column align="center" prop="real" label="真实姓名" />
      <el-table-column align="center" prop="branch" label="所在部门" />
      <el-table-column align="center" prop="name" label="角色" />
      <el-table-column align="center" prop="address" label="备注" />
      <el-table-column align="center" prop="phone" label="手机号" />
      <el-table-column align="center" prop="email" label="邮箱" />
      <el-table-column align="center" prop="state" label="状态">
        <el-button type="success" size="small">正常</el-button>
      </el-table-column>
      <el-table-column align="center" fixed="right" label="操作">
        <template #default>
          <el-button type="primary" size="small">查看 / 编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination background layout="prev, pager, next" :total="10" class="page" />
  </Content>
</template> -->

<!-- <script lang="ts" setup>
const tableData = [
  {
    date: '1',
    user: '',
    real: '',
    branch: '',
    name: '',
    address: '',
    phone: '',
    email: '',
  },
];
</script> -->

<!-- <style lang="scss" scoped>
.break {
  position: absolute;
  top: 100px;
  right: 25px;
}
.add {
  float: right;
  margin-bottom: 20px;
  margin-left: 5px;
}
.del {
  float: right;
  margin-bottom: 20px;
  margin-left: 5px;
}

::v-deep .el-table thead {
  color: #000000;
}

::v-deep .el-table th.el-table__cell.is-leaf {
  background-color: rgb(250, 250, 250);
}
</style> -->
